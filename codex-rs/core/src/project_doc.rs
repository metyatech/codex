...
fn render_js_repl_instructions(config: &Config) -> Option<String> {
    ...
    if config.features.enabled(Feature::JsReplPolling) {
        section.push_str("- Polling mode is session-based: call `js_repl` with first-line pragma `// codex-js-repl: poll=true` to get `exec_id` and `session_id`; provide `session_id=<id>` in later `js_repl` calls to reuse that session state.\n");
