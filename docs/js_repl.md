...
When enabled, `js_repl` accepts `poll=true` in the first-line pragma and returns both `exec_id` and `session_id`. Reuse polling state by passing `session_id=<id>` in later `js_repl` pragmas. Use `js_repl_poll` with `exec_id` until `status` becomes `completed` or `error`.
...
- Optional first-line pragma:
  ...
  - `// codex-js-repl: poll=true session_id=my-session timeout_ms=15000`
...
5. Optional: reuse session state by submitting another polled `js_repl` call with `session_id=<id>`.
